...
PERPLEXITY_PROMPT = """
Given a user's question and their current answers about a visualization, your task is to gather the most credible, relevant, and up-to-date evidence from the web to deepen their understanding, correct misconceptions if needed, and surface diverse perspectives or missing context.
"""

PERPLEXITY_PROMPT_PROBER = """
Given a user's question and their current answers about a visualization, your task is to further probe the user by asking more questions backed by credible, relevant, and up-to-date evidence from the web to deepen their understanding, correct misconceptions if needed, and surface diverse perspectives or missing context.
"""

PERPLEXITY_FORMAT_INSTRUCTIONS = """
...
Each answer must be written in paragraph form.
The output should ONLY include the JSON snippet above and nothing else. Do not put json in the snippet.
"""
# SYSTEM_PROMPT = """
...
class InsightExplorer(object):
    ...
    def __init__(self):
        self.perplexity_cli = perplexity.Client()

    def extract_citations(self, text: str) -> list[int]:
        print("extract", text)
        return list({int(match) for match in re.findall(r'\[(\d+)\]', text)})

    def generate(self, goal: Goal, answers: list[str], prompts: Prompt, n: int): 
        ...
        for i in range(len(prompts)):
            ...
            \n Answer: {answers[i]}
            """
        user_prompt += PERPLEXITY_FORMAT_INSTRUCTIONS

        user_prompt += f"""
        Generate {n}. All answers should have citations.
        """

        print(user_prompt)

        response = self.perplexity_cli.search(user_prompt,mode='auto', model=None, sources=['web'], files={}, stream=False, language='en-US', follow_up=None, incognito=False)

        # Clean response and return perplexity object
        final_answer_raw = response['text'][-1]['content']['answer']
        answer_json = json.loads(final_answer_raw)

        # Getting References
        web_results = answer_json.get('web_results', [])
        ...
        raw_answer_text = answer_json['answer']
        raw_answer_text = clean_code_snippet(raw_answer_text)
        answers = json.loads(raw_answer_text)

        print("answers: ", answers)

        # Convert to Answer objects
        answers = [Answer(index=a['index'], text=a['answer'], references=[str(r) for r in self.extract_citations(a["answer"])]) for a in answers]
        perplexity_insights = PerplexityResearch(answers=answers, references=references, related_queries=[])
    ...
    #     search_phrases = ["Most efficient car types", "Factors that affect car prices"]
    ...
    def research(self, goal: Goal, answers: list[str], prompts: Prompt, n: int): 
        user_prompt = PERPLEXITY_PROMPT_PROBER
